# first-godot-game — 开发说明

在修改本仓库前，必须完整阅读 PROJECT_GOAL.md。该文件是产品目标、版本进度、
Issue 状态和持续开发规则的唯一长期来源。

## 工作规则

- Godot 4.7，GDScript，GL Compatibility 渲染模式。
- 场景放 `scenes/`，脚本放 `scripts/`，UI 放 `ui/`，资源放 `assets/`。
- 每次只完成一个可独立使用的最小版本。
- 每次修改必须在 Godot 编辑器中验证无脚本错误。
- 每次完成后更新 PROJECT_GOAL.md 的"当前进度"表和 README.md。
- 每轮开始时检查 GitHub Issues：可执行的回复并纳入版本，需澄清的回复问题并保持开放；
  发布后在 Issue 回复版本号、提交和结果并关闭。
- 提交信息使用语义化风格（feat/fix/chore）。
- 创建对应版本标签并推送 main 与标签。不改写已推送的历史，不丢弃已有功能。

## 开始新一轮开发

1. `git status -sb`、`git log -5 --oneline --decorate`。
2. 检查 GitHub Issues：回复→纳入→实现→发布→关闭。
3. 完整阅读 PROJECT_GOAL.md，特别关注"当前进度"表和"下一版本分析"。
4. 完成下一最小版本，同步更新 PROJECT_GOAL.md 和 README.md（双写）。

## 跨环境继续开发

产品上下文、进度和 Issue 状态全部保存在 PROJECT_GOAL.md。换电脑后克隆仓库并阅读
AGENTS.md→PROJECT_GOAL.md 即可恢复完整上下文，不依赖聊天记录。

## 项目结构

```
first-godot-game/
├── AGENTS.md            # 工作规则（本文件）
├── README.md            # 发布说明
├── PROJECT_GOAL.md      # 产品上下文（必须先读）
├── .omo/                # 自动化脚本
├── scenes/              # Godot 场景文件
├── scripts/             # GDScript 脚本
├── ui/                  # UI 场景
└── assets/              # 资源文件（图片、音频）
```
